...
try:
    ...
    # ปรับตรงนี้: ระบุชื่อโมเดลให้ชัดเจน และใช้เวอร์ชันล่าสุด
    model = genai.GenerativeModel('gemini-1.5-flash-latest') 
except Exception as e:
    st.error(f"⚠️ ปัญหาการตั้งค่า: {e}")
    model = None
...
def ask_ai_for_friend(user_message):
    ...
    try:
        ...
        # ดักจับ Error หากโมเดลยังไม่รองรับบางคำสั่ง
        return f"เรารับฟังอยู่นะเพื่อน... (ระบบกำลังปรับจูนนิดหน่อย: {e})"

# --- [ ส่วนที่เหลือของโค้ดช่างใหญ่คงเดิมได้เลยครับ ] ---
ฟังก์ชันการทำงานของ AI ] ---
def ask_ai_for_friend(user_message):
    if model is None:
        return "ตอนนี้ช่างใหญ่ทำกุญแจหาย... (กรุณาเช็ค API Key ใน Secrets)"
...
# --- [ 6. ส่วนแชท AI เพื่อนคู่คิด ] ---
st.write("---")
st.subheader("💬 พื้นที่ระบายความในใจ (ช่างใหญ่ AI)")
user_input = st.text_area("อยากระบายอะไรไหมเพื่อน?", placeholder="พิมพ์เรื่องที่เจอมาได้เลย...", key="ai_input")
...
c1, c2 = st.columns(2)
with c1:
    up_img = st.file_uploader("อัปโหลดรูป", type=["jpg", "png"], key="img")
    if up_img: st.image(up_img)
with c2:
    up_vid = st.file_uploader("อัปโหลดวิดีโอ", type=["mp4"], key="vid")
