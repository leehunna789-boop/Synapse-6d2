    ...
    if user_input:
        try:
            with st.status("🔮 กำลังซิงค์ข้อมูลจริงและ AI Matrix...", expanded=True) as status:
                # 1. AI Analysis (แก้ไข Prompt ให้แม่นยำขึ้น)
                prompt = f"Analyze the emotional valence of this text: '{user_input}'. The current weather is {env['weather']}. Return ONLY a JSON object with a single key 'v' and a value between 0.0 and 1.0."
