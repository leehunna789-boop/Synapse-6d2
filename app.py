...
# ใช้ระบบ Secrets เพื่อดึงกุญแจ API อย่างปลอดภัย (ห้ามแก้ไขส่วนนี้)
if "GEMINI_API_KEY" in st.secrets:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"])
    # ระบุชื่อรุ่นให้ชัดเจนเพื่อป้องกัน Error 404
    ...
    if user_input:
        try:
            with st.status("🔮 กำลังซิงค์ข้อมูลจริงและ AI Matrix...", expanded=True) as status:
                # 1. AI Analysis
                prompt = f"Analyze User: '{user_input}'. Based on Weather: {env['weather']}. Return ONLY JSON: {{'v': 0.0-1.0}}"
